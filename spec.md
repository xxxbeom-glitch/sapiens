...
- **`crawl_domestic`**: 탭마다 **지정 RSS만** 읽어 `domestic_market` / `global_market` / `ai_issue` 문서에 넣는다. 풀을 합쳐 LLM으로 탭을 나누지 않는다.
  ...
  - **미국증시**: `RSS_FEEDS_NEWS_CNBC_MARKETS`(Yahoo `news/rssindex` + `rss/headline` 다티커 피드 등; 기사 URL은 `yahoo.com`만).
  - **AI 이슈**: `RSS_FEEDS_NEWS_AI`(CNBC Tech + Yahoo Finance 다티커 헤드라인 `rss/headline?s=NVDA,...`).
