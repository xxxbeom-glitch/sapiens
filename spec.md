...
- **파이프라인**: RSS·네이버 등 크롤 → Claude Haiku로 기사 요약·탭 분류 → Firestore 저장. `PIPELINE_SECTION`으로 일부만 실행 가능.
...
| LLM | 기사 요약·탭 분류·시황 리포트: **Claude Haiku** (`pipeline/summarizer.py`의 `CLAUDE_HAIKU_MODEL`, 기본 `claude-3-5-haiku-20241022`). `ANTHROPIC_API_KEY` 필수. 선택 환경변수 `CLAUDE_MODEL`. |
...
- **모델**: Claude Haiku (`summarizer.py`, `CLAUDE_HAIKU_MODEL`).
...
| 요약·Claude Haiku | `pipeline/summarizer.py` |
