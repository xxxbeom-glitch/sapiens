...
def summarize_article(
    ...
    *,
    ...
    user = (
        ...
        f"{evidence_rule}"
        f"리스트형/추천형 기사 규칙:\n"
        f"- 제목·본문에 '주목', '추천', 'Top', '유망', '성장주', 'pick', '주목해야 할' 등 **특정 종목(기업) 목록**을 암시하는 표현이 있으면, "
        f"summary_points 중 최소 1개는 **기사에 나온 종목(회사)명/티커를 구체적으로 나열**해야 합니다(예: 엔비디아(NVDA)·팔란티어(PLTR)…). "
        f"종목을 '관련주/해당 기업'처럼 뭉뚱그려 대체하지 마세요.\n"
